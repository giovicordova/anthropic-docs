...
- The `source` column tags content as `"platform"`, `"code"`, `"api-reference"`, `"blog"`, `"model"`, or `"research"` for filtered search. API reference pages are auto-detected by path (`/docs/en/api/`). Blog, model, and research rows are excluded from the doc generation swap (they persist across doc re-crawls).
